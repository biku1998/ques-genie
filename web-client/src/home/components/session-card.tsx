...
export default function SessionCard(props: SessionPayload) {
  ...
  return (
    <Link to={`/session/${id}`}>
      <div className="p-4 rounded-lg flex flex-col relative border max-w-[305px] group hover:border-blue-500 transition-colors duration-200 ease-in-out hover:shadow-sm hover:cursor-pointer h-[208px]">
        ...
        <div className="flex items-center flex-wrap mt-4 gap-2">
          {labels
            .slice(0, labels.length > 3 ? 3 : labels.length)
            .map((label) => (
              <Badge
                variant="secondary"
        ...
        <span className="mt-auto text-xs text-slate-600">
